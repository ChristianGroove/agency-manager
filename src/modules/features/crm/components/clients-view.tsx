...
function ClientsContent({ initialData, totalCount, currentPage, spaceType, initialSettings, allCategories = [] }: { initialData: any, totalCount: number, currentPage: number, spaceType: VerticalType, initialSettings: any, allCategories?: any[] }) {
    ...
    const handleBulkDelete = async () => {
        ...
        setIsDeleting(true)
        try {
            const { deleteClientsAction } = await import("@/modules/features/crm/crm-actions")
            const result = await deleteClientsAction(Array.from(selectedIds))
            if (result.success) {
    ...
    const handleSingleDelete = async (id: string) => {
        ...
        try {
            const { deleteClientsAction } = await import("@/modules/features/crm/crm-actions")
            const result = await deleteClientsAction([id])
            if (result.success) {
