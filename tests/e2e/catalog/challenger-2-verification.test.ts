 ...
 * File: tests/e2e/catalog/challenger-2-verification.test.ts
 *
 * Direct Empirical Adversarial Testing of:
...
export const suite = {
  name: 'CHALLENGER-2: TypeScript, Security, Cartesian & Anonymous Storefront Audit',
  tier: 'Tier 5',
  ...
  tests: [
    ...
    },
    {
      ...
      fn: async () => {
        const invalidItem = {
          ...
          category: 'Test',
          base_price: 100000,
          compare_at_price: 80000, // Invalid: compare_at_price < base_price
          type: 'physical',
          classification: 'physical',
          has_variants: false,
        };
    ...
    },
    {
      ...
      fn: async () => {
        ...
        expect(durationMs).toBeLessThan(10); // Aborts immediately without building 1M objects
      },
    },
    ...
    // =========================================================================
    {
      ...
      fn: async () => {
        ...
        // Mock database containing multi-tenant items
        const mockCatalogDb = [
        ...
        // Simulate tenant-scoped query for Tenant A
        ...
        // Verify Tenant B's items are 100% hidden from Tenant A
    ...
    },
    {
      ...
      fn: async () => {
        ...
        // Tenant A tries to inject variant into Tenant B's item
    ...
    },
    {
      ...
      fn: async () => {
        ...
        // Tenant A registers SKU 'PROD-001'
        expect(registerSku(TENANT_A, 'PROD-001')).toBe(true);

        // Tenant A cannot register duplicate SKU 'PROD-001'
        expect(registerSku(TENANT_A, 'PROD-001')).toBe(false);

        // Tenant B CAN register same SKU 'PROD-001' (isolated namespaces)
    ...
    // =========================================================================
    {
      ...
      fn: async () => {
        // Postgres constraint: quotes_entity_check -> CHECK ((client_id IS NOT NULL AND lead_id IS NULL) OR (client_id IS NULL AND lead_id IS NOT NULL))
        ...
        const anonymousStorefrontQuote = {
          ...
          client_id: null, // Null client_id for public guest
          status: 'draft',
          total: 350000,
          items: [
            {
    ...
    },
    {
      ...
      fn: async () => {
        ...
        // Simulate lead payload insertion
        const leadRecord = {
          ...
          user_id: null, // No authenticated user in anonymous public session
        };
      ...
      fn: async () => {
        const publicStoreItems: UniversalCatalogItem[] = [
          ...
          } as any,
          {
            ...
            base_price: 80000,
            is_visible_in_portal: false, // Hidden
            is_active: true,
            deleted_at: null,
          } as any,
          {
            ...
            is_visible_in_portal: true,
            is_active: true,
            deleted_at: '2026-08-01T12:00:00Z', // Deleted
          } as any,
        ];
...
if (process.argv[1] && process.argv[1].endsWith('challenger-2-verification.test.ts')) {
  runSuite().then((res) => {
    ...
    if (!res.passed) {
      ...
      console.log('\nAll Challenger 2 tests passed successfully with 0 errors!\n');
      process.exit(0);
    }
  });
}
