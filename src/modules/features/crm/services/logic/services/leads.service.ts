...
export class LeadsService {
    ...
    async getPaginated(params: {
        ...
        // 1. If a specific channel is selected, we use ONLY that channel (if allowed).
        // 2. If 'All' is selected (null connectionId) and user is restricted, we use ALL their allowed channels.
        // 3. Otherwise, we pass null to see everything (for admins).
        
        let effectiveConnectionIds: string[] | undefined = undefined

        if (params.connectionId && params.connectionId !== 'all') {
            // Check if specifically selected channel is in allowed list (if restricted)
            if (params.allowedChannels && params.allowedChannels.length > 0) {
                if (params.allowedChannels.includes(params.connectionId)) {
                    effectiveConnectionIds = [params.connectionId]
                } else {
                    // User is trying to access a channel they don't own
                    return { leads: [], totalCount: 0, stageCounts: {} }
                }
            } else {
                effectiveConnectionIds = [params.connectionId]
            }
        } else if (params.allowedChannels && params.allowedChannels.length > 0) {
            // User selected 'All' or is restricted -> Show all their allowed channels
            effectiveConnectionIds = params.allowedChannels
        }
