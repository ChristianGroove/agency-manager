...
export async function getTrashItems(): Promise<TrashItem[]> {
    ...
    const now = new Date()
    const GRACE_PERIOD_DAYS = 30 // Aligned with TrashPurge Job (Inngest)
    ...
    // Parallelize queries for maximum performance (SaaS Engine V2 Optimization)
    const [
        { data: clients },
        { data: services },
        { data: briefings },
        ...
        supabase.from('leads').select('id, name, deleted_at').eq('organization_id', orgId).not('deleted_at', 'is', null),
        supabase.from('services').select('id, name, deleted_at').eq('organization_id', orgId).not('deleted_at', 'is', null),
        supabase.from('briefings').select('id, deleted_at, template:briefing_templates(name)').eq('organization_id', orgId).not('deleted_at', 'is', null),
        supabase.from('invoices').select('id, invoice_number, deleted_at').eq('organization_id', orgId).not('deleted_at', 'is', null),
        supabase.from('quotes').select('id, number, deleted_at').eq('organization_id', orgId).not('deleted_at', 'is', null),
        orgDetails?.organization_type !== 'client' 
            ? supabase.from('organizations').select('id, name, deleted_at').eq('acquired_by_reseller_id', orgId).not('deleted_at', 'is', null)
            : Promise.resolve({ data: null })
    ])

    // Process results
    if (clients) clients.forEach(c => results.push({ id: c.id, type: 'client', name: c.name, deleted_at: c.deleted_at!, days_left: calculateDaysLeft(c.deleted_at!), original_table: 'clients' }))
    if (services) services.forEach(s => results.push({ id: s.id, type: 'service', name: s.name, deleted_at: s.deleted_at!, days_left: calculateDaysLeft(s.deleted_at!), original_table: 'services' }))
    if (briefings) briefings.forEach((b: any) => results.push({ id: b.id, type: 'briefing', name: b.template?.name || "Briefing", deleted_at: b.deleted_at!, days_left: calculateDaysLeft(b.deleted_at!), original_table: 'briefings' }))
    if (invoices) invoices.forEach(i => results.push({ id: i.id, type: 'invoice', name: `Factura ${i.invoice_number}`, deleted_at: i.deleted_at!, days_left: calculateDaysLeft(i.deleted_at!), original_table: 'invoices' }))
    if (quotes) quotes.forEach(q => results.push({ id: q.id, type: 'quote', name: `Cotización ${q.number}`, deleted_at: q.deleted_at!, days_left: calculateDaysLeft(q.deleted_at!), original_table: 'quotes' }))
    if (childOrgs) childOrgs.forEach(o => results.push({ id: o.id, type: 'organization', name: o.name, deleted_at: o.deleted_at!, days_left: calculateDaysLeft(o.deleted_at!), original_table: 'organizations' }))
