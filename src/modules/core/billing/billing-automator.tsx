                ...
                if (result.success && 'count' in result && (result.count || 0) > 0) {
                    ...
                    if (result.error) {
                        ...
                        toast.error("Error en Automatización de Facturación", {
                            description: `Detalle: ${(result.error as any).message || (typeof result.error === 'object' ? JSON.stringify(result.error, Object.getOwnPropertyNames(result.error)) : String(result.error))}`
                        })
                    }
                }
            }
