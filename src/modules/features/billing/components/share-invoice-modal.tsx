                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {[
                            ...
                            { id: 'download', label: 'Descargar', icon: Download, color: 'text-zinc-600', activeBg: 'bg-zinc-100', activeBorder: 'border-zinc-400' },
                        ].map(opt => (
                            <button
                                key={opt.id}
                            ...
                            >
                                <opt.icon className={cn("h-6 w-6", channel === opt.id ? opt.color : "text-zinc-400")} />
                                <span className={cn("text-xs font-semibold", channel === opt.id ? "text-zinc-900" : "text-zinc-400")}>{opt.label}</span>
                            </button>
                        ))}
                    </div>

                    {/* Dynamic Content Area */}
                    <div className="bg-zinc-50 rounded-xl p-4 min-h-[100px] flex flex-col justify-center border border-zinc-100">
                        {channel === 'whatsapp' && (
                            <div className="text-center space-y-1">
                                <p className="text-sm font-medium text-zinc-900">Enviar por WhatsApp</p>
                                <p className="text-xs text-zinc-500">Se abrirá el chat con {client.name} con un mensaje pre-redactado y el enlace.</p>
                            </div>
                        )}
                        {channel === 'email' && (
                            <div className="text-center space-y-1">
                                {onSendEmail ? (
                                    <>
                                        <p className="text-sm font-medium text-zinc-900">Enviar por Sistema</p>
                                        <p className="text-xs text-zinc-500">Enviaremos un correo oficial con el PDF adjunto y la marca de tu empresa.</p>
                                    </>
                                ) : (
                                    <>
                                        <p className="text-sm font-medium text-zinc-900">Enviar por Correo</p>
                                        <p className="text-xs text-zinc-500">Se abrirá tu cliente de correo predeterminado con el asunto y enlace listos.</p>
                                    </>
                                )}
                            </div>
                        )}
                        {channel === 'download' && (
                            <div className="space-y-3">
                                <div className="text-center space-y-1">
                                    <p className="text-sm font-medium text-zinc-900">Descargar PDF</p>
                                    <p className="text-xs text-zinc-500">Genera un PDF ultraliviano de la factura. No ocupa espacio en el servidor.</p>
                                </div>
                                <div className="flex items-center gap-2 bg-white border border-zinc-200 rounded-lg p-2 max-w-sm mx-auto">
                                    <p className="text-[10px] text-zinc-400 truncate flex-1 font-mono">{invoiceUrl}</p>
