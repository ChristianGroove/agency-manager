...
import { Plus, Trash2, ArrowUp, ArrowDown, Loader2, Sparkles, X, FileText } from "lucide-react"
...
import { createFormTemplate, updateFormTemplate } from "@/modules/core/forms/actions"
import { toast } from "sonner"
import { cn } from "@/lib/utils"

// Alias for cleaner code, future-proof
type FormField = BriefingField
type FormFieldType = BriefingFieldType
...
export function FormBuilderSheet({
    open,
    onOpenChange,
    templateToEdit,
    onSuccess
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (templateToEdit) {
                await updateFormTemplate(templateToEdit.id, {
                    name: template.name,
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground mt-0.5">
                                        Configura los campos y preguntas
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-8 pb-20 space-y-6">
                            ...
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        placeholder="Ej. Ficha Médica, Checklist de Inspección..."
                                        className="h-11"
                                    />
                                </div>
                                <div className="space-y-2">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center justify-between">
                                    <Label className="text-base font-semibold">Campos ({template.structure.length})</Label>
                                ...
                                {template.structure.length === 0 ? (
                                    <Card className="p-8 text-center border-dashed">
                                        <p className="text-sm text-muted-foreground">
                                            No hay campos. Haz clic en "Agregar" para empezar.
                                        </p>
                                    </Card>
                                ) : (
...
// Field Editor Component (Same logic, cleaner styling if needed)
    ...
    return (
        <Card className="p-4 hover:shadow-md transition-shadow">
            <div className="flex items-start gap-3">
                            ...
                            <div className="space-y-2">
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        ...
                                        {/* Removed color/typography specific types to be more generic, or keep if useful */}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="space-y-2">
                                        {field.options && field.options.length > 0 ? (
                                            ...
                                            <p className="text-xs text-muted-foreground italic">Agrega opciones...</p>
                                        )}
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <Input
                                            ...
                                            className="h-8 text-sm"
                                        />
                                        <Button size="sm" onClick={handleAddOption} disabled={!newOption.trim()} className="h-8 shrink-0">
                ...
                <Button size="icon" variant="ghost" onClick={onDelete} className="h-8 w-8 text-red-500 hover:bg-red-50 shrink-0">
...
function LivePreview({ template }: { template: { name: string; description: string; structure: FormField[] } }) {
    if (!template.name) {
        return (
            ...
            <div className="space-y-6">
                {template.structure.map((field) => (
                    <div key={field.id} className="space-y-2">
                        ...
                        {field.type === 'text' && <Input placeholder="Respuesta..." />}
                        {field.type === 'textarea' && <Textarea placeholder="Escribe tu respuesta..." rows={4} />}
                        ...
                        {field.type === 'upload' && <Input type="file" />}
                    </div>
                ))}
            </div>
