...
export function PortalServicesTab({ token, services, invoices, briefings, onPay, onViewInvoice, initialBriefingId, onBriefingClosed }: PortalServicesTabProps) {
    ...
    return (
        <div className="max-w-4xl mx-auto w-full mb-12 space-y-6 animate-in fade-in duration-500">
            ...
            {services.length === 0 ? (
                <div className="text-center py-12 bg-white dark:bg-zinc-900 rounded-xl border border-dashed border-zinc-200 dark:border-white/10">
                    <p className="text-gray-500 dark:text-zinc-400">{t('portal.services_tab.empty_state')}</p>
                </div>
            ) : (
