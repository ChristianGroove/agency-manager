...
export const InvoiceTemplate = forwardRef<HTMLDivElement, InvoiceTemplateProps>(
    ({ invoice, settings, brandingSettings }, ref) => {
        return (
            <div
                ref={ref}
                className="w-full bg-white text-zinc-900 shadow-lg border border-zinc-200 relative overflow-hidden rounded-xl mx-auto print:shadow-none print:border-none print:rounded-none"
                style={{
                    width: '816px',
                    height: '1056px',
                    padding: '32px',
            ...
            >
                ...
                <div className="flex flex-col h-full relative z-10">
                    ...
                    <div className="flex justify-between items-start mb-6 pb-4 border-b-2 border-zinc-900">
                        <div className="flex flex-col items-center">
                            ...
                            {(settings?.main_logo_light_url || settings?.document_logo_url || settings?.main_logo_url || settings?.portal_logo_url || settings?.agency_logo) ? (
                                ...
                                <div className="h-14 flex items-center justify-center bg-zinc-100 px-4 rounded text-xs text-zinc-400 font-medium">
                                    [ Logo ]
                                </div>
                            )}
                        </div>
                        <div className="text-right">
                            ...
                            <p className="text-lg font-bold text-zinc-900 mb-1">No. {invoice.number}</p>
                            <p className="text-xs text-zinc-600">Fecha: {new Date(invoice.date).toLocaleDateString()}</p>
                        </div>
                    </div>
                    ...
                    {(invoice.metadata?.cycle_period || invoice.is_late_issued) && (
                        <div className="mb-6 text-left max-w-lg border-l-4 border-zinc-300 pl-4 py-1">
                            ...
                            {invoice.is_late_issued && (
                                <p className="text-[10px] text-zinc-500 uppercase tracking-wide mt-1 font-medium">
                    ...
                    <div className="grid grid-cols-2 gap-6 mb-4">
                        <div>
                            ...
                            <p className="font-bold text-base text-zinc-900">{invoice.emitter?.legal_name || invoice.emitter?.display_name || settings?.company_name || settings?.agency_name}</p>

                            {(invoice.emitter?.identification_number || settings?.company_nit) &&
                                <p className="text-sm text-zinc-700">NIT: {invoice.emitter?.identification_number || settings?.company_nit}</p>
                            }
                            ...
                            {(settings?.company_address || settings?.agency_address || invoice.emitter?.address) &&
                                <p className="text-sm text-zinc-700">{settings?.company_address || settings?.agency_address || invoice.emitter?.address}</p>
                            }

                            {(settings?.company_email || settings?.agency_email || invoice.emitter?.email) &&
                                <p className="text-sm text-zinc-700">{settings?.company_email || settings?.agency_email || invoice.emitter?.email}</p>
                            }

                            {(settings?.company_phone || settings?.agency_phone || invoice.emitter?.phone) &&
                                <p className="text-sm font-semibold text-zinc-900 mt-1">Cel: {settings?.company_phone || settings?.agency_phone || invoice.emitter?.phone}</p>
                            }

                            {/* Siempre forzar que la web provenga del ADN si existe */}
                            {(settings?.agency_website || settings?.website) &&
                                <p className="text-sm text-zinc-700 mt-0.5">{settings.agency_website || settings.website}</p>
                            }
                        </div>
                        <div>
                            <h3 className="text-[10px] font-bold mb-1.5 uppercase text-zinc-500 tracking-wider">Para:</h3>
                            <p className="font-bold text-base text-zinc-900">{invoice.client?.name}</p>
                            <p className="text-sm text-zinc-700">{invoice.client?.company_name}</p>
                            <p className="text-sm text-zinc-700">NIT/CC: {invoice.client?.nit}</p>
                            <p className="text-sm text-zinc-700">{invoice.client?.address}</p>
                            <p className="text-sm text-zinc-700">{invoice.client?.email}</p>
                            <p className="text-sm font-semibold text-zinc-900 mt-1">Cel: {invoice.client?.phone}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex-shrink-0">
                        ...
                        <div className="mb-6 rounded-lg overflow-hidden border border-zinc-200">
                            <div className="bg-zinc-50 text-zinc-700 p-3 grid grid-cols-12 font-bold text-xs uppercase tracking-wider border-b border-zinc-200">
                            ...
                            <div className="bg-white">
                                {invoice.items.map((item: any, index: number) => (
                                    <div key={index} className="grid grid-cols-12 p-3 text-sm border-b border-zinc-100 last:border-b-0 hover:bg-zinc-50/30 transition-colors">
                                        ...
                                        <div className="col-span-2 text-right font-bold text-zinc-900 text-sm">${(item.price * item.quantity).toLocaleString()}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end mb-6">
                            <div className="w-1/2 border-t-2 border-zinc-900 pt-4">
                                <div className="flex justify-between mb-2">
                                    ...
                                    <span className="text-base font-bold text-zinc-900">${invoice.total.toLocaleString()} COP</span>
                                </div>
                                <div className="flex justify-between pt-2 border-t border-zinc-300">
                                    <span className="text-base font-bold text-zinc-900">TOTAL:</span>
                                    <span className="text-xl font-bold text-zinc-900">${invoice.total.toLocaleString()} COP</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mt-auto mb-2">
                        <h4 className="text-[10px] font-bold uppercase text-zinc-500 tracking-wider mb-1.5">Métodos de Pago</h4>
                        <div className="grid grid-cols-3 gap-2">
                            {settings?.payment_methods?.map((method: any) => {
                                ...
                                return (
                                    <div key={method.id} className="p-2 rounded-lg bg-zinc-50 border border-zinc-300 flex items-center justify-between gap-2 overflow-hidden">
                                        <div className="flex items-center gap-2 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                <p className="text-[10px] font-bold text-zinc-900 truncate" title={method.title}>
                                        ...
                                        {isGateway ? (
                                            ...
                                            <button
                                                className="p-1 hover:bg-zinc-200 rounded text-zinc-500 hover:text-zinc-900 transition-colors flex-shrink-0"
                                                title="Copiar"
                            ...
                            {(!settings?.payment_methods || settings.payment_methods.length === 0) && (
                                <>
                                    ...
                                    {settings?.bancolombia_account && (
                                        <div className="p-2 rounded-lg bg-zinc-50 border border-zinc-300 flex items-center justify-between gap-2">
                                            <div className="flex items-center gap-2 min-w-0">
                                                <img src="/payment-methods/bancolombia.png" alt="Bancolombia" className="h-4 w-4 object-contain flex-shrink-0" />
                                                <div className="min-w-0">
                                                    <p className="text-[10px] font-bold text-zinc-900 truncate">{settings.bancolombia_account}</p>
                                                </div>
                                            </div>
                                            <button className="p-1 hover:bg-zinc-200 rounded text-zinc-500 hover:text-zinc-900 transition-colors flex-shrink-0" title="Copiar">
                                                <span className="sr-only">Copiar</span>
                                                <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>
                                            </button>
                                        </div>
                                    ...
                                    {settings?.bre_b_number && (
                                        <div className="p-2 rounded-lg bg-zinc-50 border border-zinc-300 flex items-center justify-between gap-2">
                                            <div className="flex items-center gap-2 min-w-0">
                                                ...
                                                <div className="min-w-0">
                                                    <p className="text-[10px] font-bold text-zinc-900 truncate">{settings.bre_b_number}</p>
                                                </div>
                                            </div>
                                            <button className="p-1 hover:bg-zinc-200 rounded text-zinc-500 hover:text-zinc-900 transition-colors flex-shrink-0" title="Copiar">
                                                <span className="sr-only">Copiar</span>
                                    ...
                                    {settings?.nequi_number && (
                                        <div className="p-2 rounded-lg bg-zinc-50 border border-zinc-300 flex items-center justify-between gap-2">
                                            <div className="flex items-center gap-2 min-w-0">
                                                ...
                                                <div className="min-w-0">
                                                    <p className="text-[10px] font-bold text-zinc-900 truncate">{settings.nequi_number}</p>
                                                </div>
                                            </div>
                                            <button className="p-1 hover:bg-zinc-200 rounded text-zinc-500 hover:text-zinc-900 transition-colors flex-shrink-0" title="Copiar">
                                                <span className="sr-only">Copiar</span>
                                    ...
                                    {settings?.daviplata_number && (
                                        <div className="p-2 rounded-lg bg-zinc-50 border border-zinc-300 flex items-center justify-between gap-2">
                                            <div className="flex items-center gap-2 min-w-0">
                                                ...
                                                <div className="min-w-0">
                                                    <p className="text-[10px] font-bold text-zinc-900 truncate">{settings.daviplata_number}</p>
                                                </div>
                                            </div>
                                            <button className="p-1 hover:bg-zinc-200 rounded text-zinc-500 hover:text-zinc-900 transition-colors flex-shrink-0" title="Copiar">
                                                <span className="sr-only">Copiar</span>
                                    ...
                                    {settings?.paypal_link && (
                                        <div className="p-2 rounded-lg bg-zinc-50 border border-zinc-300 flex items-center justify-between gap-2">
                                            <div className="flex items-center gap-2 min-w-0">
                                                ...
                                                <div className="min-w-0">
                                                    <p className="text-[10px] font-bold text-zinc-900 truncate">Link de pago</p>
                                                </div>
                                            </div>
                                            <a
                    ...
                    <div className="pt-4 border-t border-zinc-100">
                        <p className="text-[9px] text-zinc-400 text-center leading-tight max-w-2xl mx-auto whitespace-pre-wrap">
