...
async function auditOliverMonthly() {
    console.log('--- AUDIT: Oliver Monthly Services ---');
    ...
    const { data: services } = await supabase
        ...
        .eq('status', 'active')
        .neq('frequency', 'quarterly'); // Exclude the fixed one

    console.log(`Found ${services.length} monthly services.`);
    services.forEach(s => {
