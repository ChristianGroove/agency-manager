...
export async function generateContractWithAi({
    ...
    // 2. Execute AI Task
    console.log("[CONTRACT_GEN] ðŸš€ Calling AI Engine with data:", {
        orgId,
        clientId,
        serviceIds,
    ...
    console.log("[CONTRACT_GEN] ðŸ“¨ AI Engine Response:", {
    ...
    if (!response.success) {
        console.error("[CONTRACT_GEN] âŒ AI Engine failed:", response.error)
        throw new Error(response.error || "Error al generar contrato con IA")
    }

    const result = response.data
    console.log("[CONTRACT_GEN] ðŸŽ¯ Final result to return:", result)
    ...
    try {
        ...
        if (saveError) {
            ...
            console.log(`[CONTRACT_GEN] âš¡ Inngest Event emitted for Contract ${contractRecord.id}`);
        }
    } catch (asyncErr) {
        console.error("[CONTRACT_GEN] âš ï¸ Post-generation orchestration failed:", asyncErr);
    }
...
export async function getClientsList() {
    ...
    if (error) {
        console.error("[CONTRACT_GEN] âŒ Error fetching clients:", error)
        throw error
    }

    console.log(`[CONTRACT_GEN] âœ… Fetched ${clients?.length || 0} clients for Org ${orgId}`)
    return clients
}
...
export async function getServicesList() {
    const orgId = await getCurrentOrganizationId()
    if (!orgId) {
        console.warn("[CONTRACT_GEN] âŒ No organization context for getServicesList")
        return []
    }
    ...
    if (error) {
        console.error("[CONTRACT_GEN] âŒ Error fetching services:", error)
        throw error
    }

    console.log(`[CONTRACT_GEN] âœ… Fetched ${services?.length || 0} services for Org ${orgId}`)
    return services
}
