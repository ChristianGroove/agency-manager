...
export function AIVerificationCard({ result, loading }: AIVerificationCardProps) {
    if (loading) {
        return (
            <div className="bg-white border border-zinc-100 rounded-xl p-6 shadow-sm animate-pulse space-y-3">
            ...
            {result.findings.length > 0 && (
                <div className="space-y-3 pl-[3.25rem]">
                    {result.findings.map((finding, idx) => (
                        <div key={idx} className="bg-white/60 p-3 rounded-lg border border-zinc-100/50 text-sm">
