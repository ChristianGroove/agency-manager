...
import { getRestoClientOrders, RestoOrderHistoryItem } from "@/modules/features/portal/components/b2c-restaurant-template/actions/resto-orders-actions"
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h3 className="font-bold text-gray-900 dark:text-white">Historial de Consumo</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Métricas clave y cronología de pedidos B2C.</p>
                </div>
            </div>

            {/* Metrics KPI Grid */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white dark:bg-white/5 p-5 rounded-xl border border-gray-100 dark:border-white/10 shadow-sm flex items-center justify-between">
                    <div>
                        <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Monto Gastado (LTV)</p>
                        <h4 className="text-2xl font-bold text-gray-900 dark:text-white">${totalSpent.toLocaleString('es-CO')}</h4>
                    </div>
                    <div className="w-12 h-12 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 rounded-full flex items-center justify-center">
                ...
                <div className="bg-white dark:bg-white/5 p-5 rounded-xl border border-gray-100 dark:border-white/10 shadow-sm flex items-center justify-between">
                    <div>
                        <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Total Pedidos</p>
                        <h4 className="text-2xl font-bold text-gray-900 dark:text-white">{orders.length}</h4>
                    </div>
                    <div className="w-12 h-12 bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 rounded-full flex items-center justify-center">
                ...
                <div className="bg-white dark:bg-white/5 p-5 rounded-xl border border-gray-100 dark:border-white/10 shadow-sm flex items-center justify-between">
                    <div>
                        ...
                        <h4 className="text-2xl font-bold text-gray-900 dark:text-white">${Math.round(averageTicket).toLocaleString('es-CO')}</h4>
                    </div>
            ...
            {/* Timeline Histórico */}
            <div className="bg-white dark:bg-white/5 rounded-xl border border-gray-100 dark:border-white/10 shadow-sm overflow-hidden min-h-[400px]">
                <div className="px-6 py-4 border-b border-gray-100 dark:border-white/10 bg-gray-50/50 dark:bg-white/5 flex items-center justify-between">
                ...
                <div className="p-6">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-60 text-center">
                            ...
                            <h4 className="text-gray-900 dark:text-white font-bold mb-1">Sin historial comercial</h4>
                        ...
                        <div className="relative border-l-2 border-gray-100 dark:border-white/10 ml-3 space-y-8 pb-4">
                            {orders.map((order, idx) => (
                                <div key={order.id} className="relative pl-6">
                                    <span className="absolute -left-[9px] top-1 w-4 h-4 rounded-full bg-white dark:bg-slate-900 border-2 border-primary ring-4 ring-white dark:ring-slate-900" />
