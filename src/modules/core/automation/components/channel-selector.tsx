...
export const ChannelSelector = memo(function ChannelSelector({
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                {renderTrigger ? (
                    ...
                    <Button
                        ...
                        aria-expanded={open}
                        className={cn(
                            "w-full justify-between h-auto min-h-12 py-2 px-3 rounded-2xl bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 hover:border-primary/50 transition-all duration-200 shadow-sm",
                            className
                        )}
                        disabled={loading}
                    >
                        <div className="flex flex-col items-start gap-0.5 text-left">
                            {multiple ? (
                                Array.isArray(value) && value.length > 0 ? (
                                    <div className="flex flex-wrap gap-1.5 py-1">
                                        {(selectedOptions as ChannelOption[]).map((o) => (
                                            <Badge 
                                                key={o.id} 
                                                variant="secondary" 
                                                className="flex items-center gap-1.5 pl-1.5 pr-2 py-1 h-6 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-none rounded-lg text-[10px] font-semibold"
                                            >
                                    ...
                                    <span className="text-slate-400 text-sm font-medium">Seleccionar canales activos...</span>
                                )
                            ) : (
                                value === 'all' || !value ? (
                                    <span className="flex items-center text-slate-600 dark:text-slate-300 font-medium h-6">
                                        <Globe className="mr-2 h-4 w-4 text-primary/70" />
                                        Todos los canales
                                    </span>
                                ) : (displaySelected && !Array.isArray(displaySelected)) ? (
                                    <span className="flex items-center text-slate-900 dark:text-white font-semibold h-6">
                                    ...
                                    <span className="text-slate-400 text-sm font-medium">Seleccionar canal...</span>
                                )
                            )}
                        </div>
            ...
            <PopoverContent className="w-[320px] p-2 rounded-2xl shadow-xl border-slate-200 dark:border-slate-800 backdrop-blur-xl bg-white/95 dark:bg-slate-950/95" align="start">
                <Command className="bg-transparent">
                    ...
                    <CommandList className="max-h-[300px] overflow-y-auto pr-1">
                        <CommandEmpty className="py-6 text-center text-sm text-slate-500">No se encontraron canales.</CommandEmpty>
                        ...
                        {!multiple && (
                            <CommandGroup heading="Opciones Globales">
                                <CommandItem
                                    ...
                                    className="rounded-xl flex items-center px-3 py-2.5 mb-1 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-900"
                                >
                                    <div className="flex items-center flex-1">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[10px] text-slate-500">Escuchar mensajes de cualquier fuente</span>
                                        </div>
                                    </div>
...
const ChannelItem = ({ option, isSelected, onSelect, multiple }: { option: ChannelOption, isSelected: boolean, onSelect: (id: string) => void, multiple: boolean }) => (
    <CommandItem
        ...
        className="rounded-xl flex items-center px-3 py-2.5 mb-1 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors"
    >
        <div className="flex items-center flex-1 min-w-0">
            <div className="h-8 w-8 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center mr-3 shrink-0">
            ...
            <div className="flex flex-col min-w-0">
                ...
                {option.subLabel && <span className="text-[10px] text-slate-500 truncate">{option.subLabel}</span>}
            </div>
        </div>
        ...
        {multiple ? (
            <div className={cn(
                ...
                isSelected ? "bg-primary border-primary scale-110 shadow-sm" : "border-slate-300 dark:border-slate-700"
            )}>
