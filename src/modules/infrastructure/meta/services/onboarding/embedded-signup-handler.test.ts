...
function setup(options: {coexistence?: boolean; existing?: any; subscriptionFails?: boolean; claimed?: boolean} = {}) {
 const mutations: any[] = []
 const query: any = {select:()=>query,eq:()=>query,neq:()=>query,
 ...
 const rpc=vi.fn(async (name:string)=>({data:name==='claim_meta_history_request' ? options.claimed!==false : null,error:null}))
 ...
 const fetcher=vi.fn(async (url:any, init:any)=>{
   ...
   else if(path.endsWith('/smb_app_data')) data={request_id:JSON.parse(init.body).sync_type+'-request'}
...
describe('Embedded Signup asset ownership and coexistence',()=>{
 ...
 it('does not repeat history requests on reconnection or concurrent claim loss',async()=>{
  const {fetcher}=setup({coexistence:true,claimed:false,existing:{id:'connection',metadata:{history_sync_request_status:'requested'}}})
