...
### B. Arquitectura de Capas & Dominios
Pixy evoluciona hacia una arquitectura de **Dominios Desacoplados** para garantizar que el crecimiento de funciones complejas no degrade la mantenibilidad del Core.

**Estructura de Dominios:**
- **Core Domain (`src/modules/core`)**: Infraestructura crítica y servicios transversales (IAM, SaaS Engine, Organizations, Settings).
- **Billing Domain (`src/modules/billing/platform`)**: Gestión financiera de la plataforma (Ingresos, Liquidaciones, Gateways, Revenue Sharing).
- **Features Domain (`src/modules/features`)**: Módulos operativos activables por Space (CRM, Billing-Feature, Marketing, etc.).

**Evolución del Core**: Se ha estandarizado la nomenclatura de acciones a `[module]-actions.ts` y fragmentado los módulos masivos (como Organizations) en sub-módulos especializados (`crud`, `provisioning`, `context`) para evitar archivos "God".
...
**Revenue Sharing (Dominios Específicos)**:
- **Cálculo por Fases**:
    - **Activation (Mes 0-6)**: 25% de comisión. Incluye servicios base.
    - **Retention (Mes 7-12)**: 15% de comisión. Requiere log de actividad del reseller.
    - **Stable (Mes 13+)**: 10% de comisión vitalicia sobre add-ons.
- **Billing Platform Engine**: Ubicado en `src/modules/billing/platform/revenue`, este motor inmutable registra transacciones y calcula comisiones automáticamente.
- **Stripe Connect & Gateways**: Gestionados en `src/modules/billing/platform/gateways`, facilitan la liquidación a cuentas Express de resellers previa aprobación administrativa.
- **Dashboard de comisiones**: Segregación por fases y seguimiento del historial de liquidaciones.
