...
function integrationConnectionsQuery(connection: unknown = {
    credentials: {
...
describe('MetaProvider', () => {
    ...
    it('uses Authorization headers instead of query tokens for social profile fetches', async () => {
        vi.stubEnv('VERCEL_ENV', 'production')
        const logSpy = vi.spyOn(console, 'log').mockImplementation(() => undefined)
        mocks.supabaseFrom.mockImplementation((table: string) => {
            if (table === 'integration_connections') {
                return integrationConnectionsQuery({
                    credentials: {
                        accessToken: 'page-token-secret',
                        pageId: 'page_secret_id',
                    },
                    metadata: {
                        asset_id: 'page_secret_id',
                        page_id: 'page_secret_id',
                    },
        ...
        const messages = await provider.parseWebhook({
            ...
            entry: [{
                id: 'page_secret_id',
                messaging: [{
                    sender: { id: 'psid_secret' },
                    recipient: { id: 'page_secret_id' },
        ...
        expect(messages).toHaveLength(1)
        expect(messages[0]).toEqual(expect.objectContaining({
            senderName: 'Client Secret',
        }))
        expect(fetchMock).toHaveBeenCalledWith(
            'https://graph.facebook.com/v21.0/psid_secret?fields=first_name,last_name,name',
            expect.objectContaining({
                headers: { Authorization: 'Bearer page-token-secret' },
            })
        )
        const profileFetchCall = fetchMock.mock.calls[0] as unknown[] | undefined
        expect(String(profileFetchCall?.[0])).not.toContain('access_token')
