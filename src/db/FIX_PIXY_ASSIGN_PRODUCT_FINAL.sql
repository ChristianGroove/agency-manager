/* ================================================================
   FIX: Assign SaaS Product to Pixy Agency - FINAL VERSION
   ================================================================ */
...
BEGIN
    ...
    SELECT id INTO pixy_product_id FROM saas_products WHERE slug = 'pixy-complete' LIMIT 1;
    
    /* If it doesn't exist, create it with ALL required fields */
    IF pixy_product_id IS NULL THEN
        INSERT INTO saas_products (name, slug, description)
        VALUES (
            'Pixy Complete', 
            'pixy-complete',
            'Full access to all modules for Pixy Agency'
        )
    ...
    ELSE
        RAISE NOTICE 'Pixy Complete product already exists with ID: %', pixy_product_id;
    END IF;
...
SELECT 
    ...
    (SELECT STRING_AGG(sm.key, ', ')
     ...
     WHERE spm.product_id = sp.id) as modules_preview
