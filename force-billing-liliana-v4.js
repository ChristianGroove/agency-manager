...
async function forceBillingLilianaV4() {
    console.log('--- Force Billing Liliana V4 (Explicit Cols) ---');
    ...
    // 3. Find Sub (Explicit Select)
    let { data: sub } = await supabase
        .from('subscriptions')
        .select('id, name, amount, client_id, status')
        .eq('client_id', clientId)
        .ilike('name', service.name)
        .maybeSingle();

    if (!sub) {
        console.log('Sub not found. Attempting Insert...');
        // Insert with Org ID, Assuming it exists.
        ...
        }).select('id, name, amount, client_id').single();
    ...
    await createInvoice(sub, orgId, '2026-01-14');
    await createInvoice(sub, orgId, '2026-02-14');
...
async function createInvoice(sub, orgId, dateStr) {
    ...
    if (error) console.error(`Error creating invoice ${dateStr}:`, error.message);
    else {
        // Cycle
