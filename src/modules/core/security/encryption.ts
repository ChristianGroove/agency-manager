...
/**
 * Encrypt a buffer for vault storage
 */
export async function encryptBuffer(buffer: Buffer, secret: string): Promise<Buffer> {
    // For simplicity and compatibility with existing vault logic, 
    // we use the provided secret or fall back to the global key
...
/**
 * Decrypt a buffer from vault storage
 */
export async function decryptBuffer(encryptedBuffer: Buffer, secret: string): Promise<Buffer> {
    const key = crypto.createHash('sha256').update(secret).digest();
