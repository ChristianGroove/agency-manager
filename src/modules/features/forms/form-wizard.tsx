...
import { FormSubmission, FormTemplate, FormTemplate as FullFormTemplate } from "@/modules/features/forms/actions"
// We reuse the types from actions or define them locally if needed. 
// For now, assuming types are compatible or we define interfaces here.
import { BriefingField } from "@/types/briefings"
...
import { saveSubmissionResponse, submitForm } from "@/modules/features/forms/actions"
...
interface FormWizardProps {
    ...
    template: FullFormTemplate
    initialResponses: Record<string, any>
}

type WizardStatus = 'intro' | 'filling' | 'submitting' | 'completed'

export function FormWizard({ submission, template, initialResponses }: FormWizardProps) {
    // ========================================
    ...
    const debouncedSave = useDebouncedCallback(async (fieldId: string, value: any) => {
        setIsSaving(true)
        try {
            await saveSubmissionResponse(submission.id, fieldId, value)
        } catch (error) {
    ...
    if (status === 'completed') {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-6 animate-in zoom-in-95 duration-500">
                ...
                {/* Close window if opened in popup, or just show text */}
            </div>
        )
    }
...
// Type definition for FieldInput props to fix implicit any
